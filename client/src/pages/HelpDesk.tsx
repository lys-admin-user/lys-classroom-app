...
const helpArticles: HelpArticle[] = [
  ...
  {
    id: "coppa-self-signup-blocked",
    title: "Under-13 Can't Create Their Own Account",
    category: "safety",
    tags: ["COPPA", "under 13", "blocked", "sign up", "create account", "child", "age", "birthdate"],
    severity: "warning",
    symptom: "While setting up your account you see: \"Students under 13 can't create their own account.\"",
    explanation: "To follow COPPA (the children's privacy law), people under 13 can't sign themselves up for LYS. A younger student's account has to be created and supervised by a trusted adult — a teacher, school, or parent.",
    steps: [
      "Ask your teacher or school to add you as a student on their account",
      "Or ask a parent to set you up through a Homeschool Parent account",
      "Once an adult creates your account, your access is managed safely on their side",
      "If you're 13 or older, double-check that the date of birth you entered is correct",
    ],
  },
  {
    id: "coppa-birthdate-required",
    title: "Why We Ask for Your Date of Birth at Sign-Up",
    category: "safety",
    tags: ["birthdate", "date of birth", "age", "COPPA", "onboarding", "sign up", "required"],
    severity: "info",
    symptom: "Onboarding asks for your date of birth before you can continue.",
    explanation: "We ask everyone — students, educators, and homeschool parents — for a date of birth during setup so we can apply the right child-safety protections. It's required for all roles so age is always checked.",
    steps: [
      "Enter your real date of birth on the first onboarding step",
      "This is used to apply COPPA protections, not for marketing",
      "If you're under 13, you'll be guided to have an adult set up your account instead",
      "You can't skip this step — it's required to finish setup",
    ],
  },
  {
    id: "onboarding-admin-role",
    title: "Admin Access Isn't Available During Sign-Up",
    category: "safety",
    tags: ["admin", "role", "campus admin", "district admin", "permissions", "sign up", "onboarding"],
    severity: "info",
    symptom: "You don't see a Campus or District Administrator option when choosing your role, or admin tools aren't available after signing up.",
    explanation: "For security, you can only choose Student, Educator, or Homeschool Parent when signing yourself up. Administrator roles are granted by your organization, not self-selected, so no one can give themselves elevated access.",
    steps: [
      "Choose Educator to get started right away",
      "Ask your school or district's LYS account owner to enable admin tools for you",
      "An existing administrator can upgrade your role from the People management section",
      "Role changes take effect the next time you sign in",
    ],
  },
  {
  ...
  {
    id: "sis-sync-fail",
    title: "SIS Integration & Rostering Not Syncing",
    category: "integration",
    tags: ["SIS", "sync", "Clever", "OneRoster", "ClassLink", "rostering", "integration", "import", "connection"],
    severity: "error",
    symptom: "Student or course data from your SIS or rostering provider (Clever, OneRoster, ClassLink) isn't updating.",
    explanation: "SIS and rostering integrations rely on external connections that can temporarily lose connectivity. Clever is fully live; OneRoster 1.1 and ClassLink connect with an OAuth2 client ID, client secret, and token URL. Common causes include expired credentials, a wrong token URL, changes to the provider configuration, or scheduled maintenance windows.",
    steps: [
      "Check the SIS Integration page for connection status",
      "For OneRoster/ClassLink, verify the Client ID, Client Secret, and Token URL are correct",
      "Use the Test button on the connection — for OneRoster/ClassLink this confirms a token can be fetched",
      "Verify the API credentials haven't expired in your SIS or rostering provider",
  ...
  {
    id: "sso-setup",
    title: "Set Up 'Sign in with Your School' (Enterprise SSO)",
    category: "integration",
    tags: ["SSO", "single sign-on", "OIDC", "Google", "Azure", "Okta", "OneLogin", "login", "district"],
    severity: "info",
    symptom: "You want staff and students to sign in with your school's identity provider instead of creating separate passwords.",
    explanation: "Organization admins and owners can configure per-organization single sign-on using OpenID Connect (OIDC). Supported providers include Google Workspace, Microsoft Entra/Azure AD, Okta, OneLogin, or any generic OIDC issuer. Users who enter a school email matching a configured domain are routed into your provider's login and are automatically provisioned with a non-privileged role (elevated roles are only granted through admin tools).",
    steps: [
      "Go to My Campus → Enterprise SSO (admin/owner only)",
      "Click 'Add connection' and pick your provider",
      "Enter the Issuer URL (e.g. https://accounts.google.com), Client ID, and Client Secret from your provider — the secret is stored encrypted",
      "Add your allowed email domains (e.g. lincoln.edu) so users are matched correctly",
      "Choose the default role for new users and whether to auto-provision accounts on first login",
      "Save, then use 'Test' to confirm the provider is reachable",
      "Register the callback URL shown by your provider as /api/sso/callback/<connection-id>",
      "Staff and students can now choose 'Sign in with your school' on the landing page",
    ],
  },
  {
  ...
  },
  {
    ...
    explanation: `Charter networks — both CMOs (Charter Management Organizations, non-profit) and EMOs (Education Management Organizations, for-profit) — use the 'charter_network' or 'network' organization type in LYS. Examples include KIPP, IDEA Public Schools, Green Dot, and Charter Schools USA. These organizations operate at the Enterprise tier ($${PLAN_PRICES.enterprise}/mo + $${SEAT_PRICES.enterprise}/seat with ${SEAT_MINIMUMS.enterprise} seat minimum) and support managing campuses across multiple states from a central headquarters. Admins can choose between a unified master dashboard showing all campuses or per-state management views.`,
    steps: [
      `Charter networks are created with type='charter_network' or 'network' and are automatically Enterprise tier ($${PLAN_PRICES.enterprise}/mo + $${SEAT_PRICES.enterprise}/seat, ${SEAT_MINIMUMS.enterprise} seat min)`,
  ...
  },
  {
    ...
    steps: [
      `Single-Campus Charter: Independent school with no parent organization. Uses type='school' or 'campus', Campus tier at $${PLAN_PRICES.campus}/mo + $${SEAT_PRICES.campus}/seat (${SEAT_MINIMUMS.campus} seat min). Full customization, makes own rules.`,
      `Traditional ISD (Public School District): Geographically bound, locally governed district with elected board. Uses type='district' with child 'school'/'campus' orgs, Enterprise tier at $${PLAN_PRICES.enterprise}/mo + $${SEAT_PRICES.enterprise}/seat (${SEAT_MINIMUMS.enterprise} seat min). Standardized calendar and curriculum.`,
      `Multi-State Charter Network (CMO/EMO): Central HQ managing schools across states. Uses type='charter_network' or 'network' with child orgs, Enterprise tier at $${PLAN_PRICES.enterprise}/mo + $${SEAT_PRICES.enterprise}/seat (${SEAT_MINIMUMS.enterprise} seat min). Examples: KIPP, IDEA, Green Dot, Charter Schools USA.`,
      `All districts and charter networks are automatically Enterprise tier ($${PLAN_PRICES.enterprise}/mo + $${SEAT_PRICES.enterprise}/seat, ${SEAT_MINIMUMS.enterprise} seat min) regardless of size`,
      `Single-campus schools can operate independently at Campus tier ($${PLAN_PRICES.campus}/mo + $${SEAT_PRICES.campus}/seat, ${SEAT_MINIMUMS.campus} seat min) with full features`,
