...
export function IconRailCharcoal() {
  ...
  const roleProfiles = {
    "Student": { name: "Jordan Lee", title: "8th Grade", img: "https://images.unsplash.com/photo-1543610892-0b1f7e6d8c1c?q=80&w=150&auto=format&fit=crop" },
    "Homeschool Parent": { name: "Sam Carter", title: "Homeschool", img: "https://images.unsplash.com/photo-1544717305-2782549b5136?q=80&w=150&auto=format&fit=crop" },
    "Educator": { name: "Elena Rivera", title: "8th Grade Teacher", img: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=150&auto=format&fit=crop" },
    "Campus Admin": { name: "Elena Rivera", title: "Campus Admin", img: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=150&auto=format&fit=crop" },
    "District Admin": { name: "Elena Rivera", title: "District Admin", img: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=150&auto=format&fit=crop" },
  };
  ...
  const isStudentOrParent = role === "Student" || role === "Homeschool Parent";
  
  const railItems = isStudentOrParent 
    ? [baseRailItems[0], bkdItem, ...baseRailItems.slice(1)]
    : [...baseRailItems, bkdItem];
  ...
  const currentProfile = roleProfiles[role as keyof typeof roleProfiles];

  return (
    <div className="lys-scope min-h-screen w-full flex bg-[#F8FAFC] font-sans overflow-hidden">
      {/* 1. ICON RAIL (Charcoal) */}
      <nav className="w-[72px] flex-shrink-0 flex flex-col items-center py-5 bg-slate-950 text-slate-400 z-30 shadow-[4px_0_24px_rgba(0,0,0,0.08)] relative">
        {/* Logo */}
            ...
            return (
              <button
                ...
                className={`relative group flex items-center justify-center w-full aspect-square rounded-xl transition-all duration-300
                  ${isActive 
                    ? "bg-white/10 text-white shadow-[0_2px_10px_rgba(0,0,0,0.1)] ring-1 ring-white/10" 
                    : "hover:bg-white/5 hover:text-slate-200 hover:scale-105 active:scale-95"
                  }`}
                style={{
        ...
        <div className="mt-auto flex flex-col gap-3 w-full px-3">
          {[
            ...
            <button key={i} className="relative group flex items-center justify-center w-full aspect-square rounded-xl hover:bg-white/5 hover:text-slate-200 hover:scale-105 active:scale-95 transition-all duration-300">
      ...
      <div className="w-[280px] flex-shrink-0 bg-white border-r border-slate-200/80 z-20 flex flex-col shadow-[2px_0_12px_rgba(0,0,0,0.03)] relative">
        ...
        <div className="h-[72px] flex items-center gap-3 px-5 border-b border-slate-100 bg-slate-50/50 backdrop-blur-sm relative">
          <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-slate-900 to-slate-800/10" />
              ...
              {(flyoutContent[activeRail as keyof typeof flyoutContent] as Array<any>).map((item, i) => (
                ...
                >
                  <item.icon className={`w-[18px] h-[18px] stroke-[1.75] transition-colors
                    ${item.active ? "text-slate-800" : "text-slate-400 group-hover:text-slate-600"}`} />
                ...
                <div key={sIdx} className="flex flex-col gap-1.5">
                  ...
                  <div className="flex flex-col gap-1">
                    {section.items.map((item: any, i: number) => (
                      <button
                        key={i}
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-[10px] text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all duration-200 group"
                      >
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 mt-auto">
          <button className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition-all duration-200 group text-left">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[13px] font-bold text-slate-900 truncate">{currentProfile.name}</span>
              <span className="text-[11px] text-slate-500 font-medium truncate">{currentProfile.title}</span>
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative bg-[#F8FAFC]">
        ...
        <header className="h-[72px] border-b border-slate-200/60 bg-white/70 backdrop-blur-md flex items-center justify-between px-8 z-10 sticky top-0">
          ...
          {/* ROLE SWITCHER */}
          <div className="flex items-center bg-slate-100/80 p-1 rounded-xl border border-slate-200/80 shadow-sm">
            {roles.map(r => (
              <button
                key={r}
                onClick={() => setRole(r)}
